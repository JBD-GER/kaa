...
export default async function ArticlePage({ params }: Props) {
  ...
  const related = articles.filter((item) => item.slug !== article.slug && item.serviceSlugs.some((service) => article.serviceSlugs.includes(service))).slice(0, 3);
      ...
      <section className="section section--white">
        <Container className="article-layout">
          ...
          <article className="article-content">
            ...
            <aside className="article-author"><strong>{article.author}</strong><p>Die KAA Redaktion bereitet Grundlagen zu KI-Automatisierung, Systemintegration und individueller Software verständlich für Unternehmensentscheider auf.</p><span>Keine erfundene Personenbiografie – redaktioneller Organisationsautor.</span></aside>
          </article>
        </Container>
      </section>

      {related.length ? (
        <section className="section"><Container><h2 style={{ marginBottom: "2rem" }}>Weiterlesen</h2><div className="article-grid">{related.map((item) => <article className="article-card" key={item.slug}><div className="article-card__meta"><span>{item.category}</span><span>{item.readingTime} Min.</span></div><h2><Link href={`/ratgeber/${item.slug}`}>{item.title}</Link></h2><p>{item.excerpt}</p><span className="article-card__link">Artikel lesen ↗</span></article>)}</div></Container></section>
      ) : null}
